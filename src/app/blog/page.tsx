      ...
      <section className="py-16">
        <div className="container mx-auto px-5 max-w-7xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {/* 1. Added 'index' here to track the position of each blog post */}
            {blogPosts.map((post, index) => {
              
              {/* 2. Dynamically calculate the image number (1 to 20). 
                  The '%' operator ensures it wraps back around to 1 if you have more than 20 blogs */}
              const imageNumber = (index % 20) + 1;
              const dynamicImageUrl = `/image/assets/bllog_thumbnail/blog-${imageNumber}.png`;
                  ...
                  <Card className="overflow-hidden border-none shadow-sm hover:shadow-md transition-shadow bg-card h-full flex flex-col">
                    <div className="relative aspect-[16/9] overflow-hidden bg-muted">
                      <Image
                        ...
                        // Tip: Only use priority for the first few visible images on the screen to boost performance
                        priority={index < 3} 
                      />
...
<CardContent className="p-6 flex flex-col flex-grow">
  ...
  <div className="mt-auto flex items-center justify-between text-xs text-muted-foreground">
    {/* Reverted this line back to standard text to fix the TypeScript error */}
